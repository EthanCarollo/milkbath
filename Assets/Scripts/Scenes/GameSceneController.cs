...
namespace MilkBath.Scenes
{
    public sealed class GameSceneController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            CreateGameUi(transitioner);
        }

        private static void CreateGameUi(SceneTransitioner transitioner)
        {
            Canvas canvas = CreateCanvas("GameCanvas");
            CreateLabel(canvas.transform, "GAME SCENE", new Vector2(0f, 120f), 42, Color.white);

            Button backButton = CreateButton(canvas.transform, "BACK TO MENU", new Vector2(0f, -10f));
            backButton.onClick.AddListener(() => transitioner.LoadScene(MainMenuSceneName));
        }
